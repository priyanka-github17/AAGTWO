    ...
    if (empty($status)) {
     ...
     // echo $paramType;
      ...
      if (!empty($userId)) {
        ...
        $mail->sendEmail();
      $response = setResponse('success', 'You are registered succesfully for the GlobalLiveSummit. Please check your email regarding instructions on how to login.');
      } else {
           $response = setResponse('error', 'You cound not be registered. Please try again.');
           return $response;
      }
     ...
     // $dateTimestamp1 = strtotime($status[0]["logoutdate"]);
      ...
      //header('location: enter.php');
            $response = setResponse('error', 'You are registered succesfully');
      return $response;
   // }
  }
